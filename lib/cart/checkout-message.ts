...
import { cartItemUnitPrice, type CartItem } from "@/store/cart";
...
function itemBlock(item: CartItem, index: number): string {
  ...
  const priceLine = item.discountPrice
    ? `~${formatPrice(item.price * item.quantity)}~ → ${formatPrice(lineTotal)}`
...
export function buildCheckoutMessage(items: CartItem[]): string {
  ...
  const discountedTotal = items.reduce((sum, item) => sum + cartItemUnitPrice(item) * item.quantity, 0);
