...
- [x] Notion aposentado — cadastro de produto nativo em `/admin/products` (criar/editar página cheia, até 4 fotos na ordem de upload, tags/material texto livre), quantidade em estoque (badge "Esgotado"/"Última unidade!" no site, teto no carrinho, revalidação de estoque no checkout), e importação em lote via CSV+ZIP exportado do Notion (parse 100% no navegador, pula duplicata por nome) — em verificação final (ver pendência abaixo)

### Pendente
- [ ] Rodar o SQL de migração (`quantity`, `notion_page_id` opcional) e verificar o fluxo completo de produtos no navegador com login real (criar, editar, importar CSV, estoque no carrinho/checkout) — ver [Log de decisões](#log-de-decisões)
- [ ] Aplicar em produção a migração de `original_price` → `discount_price` (rename de coluna + troca de valores nos produtos que já têm desconto) — implementado e a validar primeiro no ambiente de dev, ver [Log de decisões](#log-de-decisões)
...
- [ ] Ambiente de dev separado (segundo projeto Supabase, `.env.development.local`) — proposto e com o schema pronto pra rodar, mas o projeto ainda não foi criado pelo usuário.
